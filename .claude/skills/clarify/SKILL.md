...
Map-Visualizer-specific prompts to apply:
- Does the requirement add a new render mode? If so, is the output format (PNG/SVG/PDF/bytes) specified? Is the render parameter set (colormap, levels, bins, row index, etc.) fully named?
- Does the requirement touch `map_visualizer/core.py` or `map_visualizer/enums.py`? If so, are Agg-purity implications addressed (no pyplot/Qt/Tk import)?
- Does the requirement modify input validation or grid loading? If so, are all edge cases stated (empty grid, all-NaN, ragged rows, oversize input)?
- Does the requirement affect the access layer? If so, are MCP tool-name/parameter implications addressed? Are 422-mapping requirements stated for new error conditions?
...
## Sources
- User requirement: SDD pipeline stage-2 skill (clarify) for Map-Visualizer Group E.
