...
- `owner` — one of: `core-dev`, `gui-dev`, `access-dev`, `test-author`, `docs-writer`, `packaging-builder`.
...
- If a task touches `map_visualizer/core.py` or `map_visualizer/enums.py`: precede it with an "Agg-purity re-check" task (owner: core-dev; done criterion: "no pyplot/Qt/Tk import added to core.py or enums.py") as a prerequisite for every task that modifies those files.
- If a task introduces or modifies a render mode (draw_* helper): add a "GUI-canvas integration check" task (owner: gui-dev; done criterion: "GUI canvas calls the same core draw_* helper; no render math duplicated in gui/") before any downstream GUI task is marked done.
- If a task adds or modifies client-input validation: add a "422-mapping contract test" task (owner: test-author; done criterion: "typed exception maps to HTTP 422 {error, message} and MCP isError") before any other access-layer task is marked done.
...
- `.claude/instructions/sdd-constitution.md` — consulted for constitution-derived sequencing rules (Agg-purity re-check, GUI-canvas integration check, 422-mapping contract test). If missing: note the gap and apply the sequencing rules stated in Step 2 of this skill.

## Sources
- User requirement: SDD pipeline stage-4 skill (tasks) for Map-Visualizer Group E.
