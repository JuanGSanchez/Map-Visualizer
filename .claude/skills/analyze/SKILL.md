  ...
  custom:
    - id: C1
      ...
      rationale: Duplicating gate logic creates divergence (P4); `run-quality-gate`
        is the single authoritative gate for Map-Visualizer.
---
...
Map-Visualizer-specific checks (consult `.claude/instructions/sdd-constitution.md` if available):
- If any FR touches `map_visualizer/core.py` or `map_visualizer/enums.py`: verify an "Agg-purity re-check" task exists in tasks.md (owner: core-dev).
- If any FR introduces or modifies a render mode (draw_* helper): verify a "GUI-canvas integration check" task exists in tasks.md (owner: gui-dev).
- If any FR adds or modifies client-input validation: verify a "422-mapping contract test" task exists in tasks.md (owner: test-author).
...
MV-specific checks:
  Agg-purity re-check task: COVERED | N/A | GAP — <detail>
  GUI-canvas integration check task: COVERED | N/A | GAP — <detail>
  422-mapping contract test task: COVERED | N/A | GAP — <detail>
...
- `.claude/instructions/sdd-constitution.md` — consulted for MV-specific checks in Step 4. If missing: skip MV-specific checks and note the gap in the report.

## Sources
- User requirement: SDD pipeline stage-5 skill (analyze) for Map-Visualizer Group E.
